...
class GrupoController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Grupo']))
		{
			...
			$model->id_grupo = $this->getClaveGrupo($model);			
			...
			try{
				...
				$model = $model::model()->find("id_grupo=:id", array(":id"=>$model->id_grupo));
				
				if($model != null){
					$this->redirect(array('view2','id'=>$model->id_grupo));
				}
	...
	public function getClaveGrupo($model){
		
		$ciclo = $model->ciclo;		
		$seccion = $model->grado0->seccion0->clave;		
		$grado = $model->grado0->numero;
		$grupo = $model->grupo;
		
		return $ciclo . $seccion . $grado . $grupo;
	}
}
